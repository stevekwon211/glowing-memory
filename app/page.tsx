    ...
    <main className={styles.mainContainer}>
      <div className={styles.contentBox}>
        ...
        <div className={styles.languageSelectorTop} onClick={toggleLanguage}>
          <span className={language === "한국어" ? styles.activeLanguage : ""}>
            한국어
          </span>
          {" / "}
          <span className={language === "EN" ? styles.activeLanguage : ""}>
            EN
          </span>
        </div>

        {/* Computer Art link at top left */}
        <Link href="/ca" className={styles.creativeArchiveLink}>
          <span>ca</span>
        </Link>
        ...
        {openOverlays.map((id) => (
          ...
          >
            {id === "about" && (
              <div className={styles.aboutMeContent}>
                <h2 className={styles.aboutTitle}>
                  {language === "EN"
                    ? aboutContent.title.en
                    : aboutContent.title.ko}
                </h2>
                <p className={styles.aboutDescription}>
                  {(language === "EN"
                    ? aboutContent.description.en
                    : aboutContent.description.ko
                  ).map((item, index) =>
                    item.type === "text" ? (
            ...
            {id === "logs" && (
              <div className={styles.mindLogsContent}>
                {writingLinks
                  ...
                  .filter(
                    (writing) => !(language === "EN" && writing.hideInEnglish)
                  ) // Filter out entries with hideInEnglish when language is EN
                  .sort(
                    (a, b) =>
                  ...
                  .map((writing) => (
                    <div key={writing.id} className={styles.writingItem}>
                      <a
                        href={
                          language === "EN" ? writing.url.en : writing.url.ko
                        }
                        target="_blank"
                        rel="noopener noreferrer"
            ...
            {id === "slides" && (
              <div
                className={styles.mindSlidesContent}
                    ...
                    .map((item) => (
                      <div
                        ...
                        onClick={(e) => {
                          ...
                          setSelectedImage(
                            selectedImage === item.id ? null : item.id
                          );
                        }}
                      >
                        <img
